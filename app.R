...
ui <- fluidPage(navbarPage(
           ...
           sidebarLayout(
             sidebarPanel(
               ...
               sliderInput(
                 ...
                 max = as.Date("2020-05-05", "%Y-%m-%d"),
                 value = c(as.Date("2020-02-25"), as.Date("2020-05-05")),
                 timeFormat = "%Y-%m-%d"
               )
             ),
  ...
  tabPanel("Regions",
           sidebarLayout(
             sidebarPanel(
               ...
               checkboxInput(
                 "check_reg_3","New cases", value= F),
               checkboxInput(
                 "check_reg_4","New cases variation (+%)", value= F),
  ...
  tabPanel("About the tool",
           mainPanel(
             ...
             p("I am especially thankful to the ", strong("Datadista"), " team who have facilitated a clean version of the data in their", a("GitHub repository. ", href= "https://github.com/datadista/datasets/tree/master/COVID%2019"), "They provide a more detailed list of the original sources", a("here, ", href = "https://github.com/datadista/datasets/blob/master/COVID%2019/readme.md"), "being the main ones:"),
             ...
             img(src = "global_view.JPG", height=285,width=836),
             p("On the top of the screen we can see four tabs (Sick, Dead, Regions and About the tool). On the left part of the screen we can see the control panel and, on the left, the plot panel. The image corresponds to the Sick tab, which is the one opened by default."),
             h3("Sick tab"),
             ...
             p("- ",strong("New cases: "),"Plots the number of people that get infected each day. The number of sick people is displayed in the y axis (using a linear scale) and the date, in the x axis. Using the previous example, if the first day there are 10 sick people and on the second day there are 10 new more cases, the numbers on the plot will be 10 and 10, respectively."),
             p("- ",strong("New cases variation (+%): "),"It is related to the previous plot. It displays the percertange of new infections with respect to the previous day against the date. That is, the percentual increment in the number of new cases. The percertange is plotted in the y axis and the date, again, in the x axis. The percertange represented is the difference between the new cases of one date and the previous one, all divided by the new cases of the previous one. The following table offers an example."),
             tableOutput('tbl'),
             ...
             img(src = "sick_tab.JPG", height = 287, width = 832),
             h3("Dead tab"),
             ...
             h3("Regions tab"),
             p("The Regions tab is concerned with the number of infections that have taken place in each region of Spain. These regions are called Comunidades Autonomas (or CC.AA.). The third tab of the tool is pretty similar to the previous two, as it can be seen in the image below. The only difference is that the user can select among the different CC.AA. in the new box of the control panel."),
             img(src="region_tab.JPG", height = 331,width = 834)
             )
           )
))
